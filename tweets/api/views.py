...
def tweet_delete_view(request, tweet_id, *args, **kwargs):
	...
	if not qs.exists():
		return Response({'message':'You cannt delete this tweet'}, status=401)
	obj = qs.first()
	obj = qs.delete()
...
def tweet_list_view(request, *args, **kwargs):
	...
	username = request.GET.get('username') #?username=lama
	if username != None:
...
def tweet_create_view_pure_django(request, *args, **kwargs):
	...
	if form.is_valid():
		...
		obj.save()
		if request.is_ajax():
			return JsonResponse(obj.serializer(), status=201) #status 201 for created items
...
def tweet_list_view_pure_django(request, *args, **kwargs):
	...
	"""
	qs = Tweet.objects.all()
	tweets_list = [x.serializer() for x in qs]
	data = {
	'isUser': False,
